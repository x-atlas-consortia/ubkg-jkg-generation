...
def getargs() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description='Builds ontology files in '
                                                 'JKGEN format from the ingest-validation_tools repo.',
...
def build_parent_node_list(sab: str, dict_index: dict) -> list:
    ...
    for parent in dict_index:
        ...
        node_synonyms = ''
        node_dbxrefs = ''
        dict_node = {'node_id': node_id, 'node_label': node_label, 'node_definition': node_definition,
    ...
    for field in yaml_dict_field_nodes:
        ...
        for node in field_nodes:
            ...
            if len(match) == 0:
                ...
                dict_node = {'node_id': node_id, 'node_label': node_label,
...
def add_nodes(path: str, list_nodes: list):
    ...
    with open(path, 'a') as out:
        for field in list_nodes:
            ...
            out.write(f'{node_id}\t{node_label}\t{node_definition}'
...
def main():
    ...
    ulog = ubkgLogging(log_dir=log_dir, log_file='hmfield2jkgen.log')
    ...
    repo_dir = cfg.get_value(section='Directories', key='repo_dir')
    ...
    # isa relationships.
    all_parent_idx = {'sab': int(cfg.get_value(section='Indexes', key='SAB_PARENT')),
                      'field': int(cfg.get_value(section='Indexes', key='FIELD_PARENT')),
                      'type': int(cfg.get_value(section='Indexes', key='TYPE_PARENT')),
                      'entity': int(cfg.get_value(section='Indexes', key='ENTITY_PARENT')),
                      'assay': int(cfg.get_value(section='Indexes', key='ASSAY_PARENT')),
                      'schema': int(cfg.get_value(section='Indexes', key='SCHEMA_PARENT'))}

    # Get the base URL to the UBKG API.
    ubkg_url = cfg.get_value(section='URL', key='ubkg_url')
    ...
    list_encoded_fields = build_node_list(ulog=ulog, sab=args.sab, yaml_dict_field_nodes=dict_fields,
    ...
    list_encoded_types = build_node_list(ulog=ulog, sab=args.sab, yaml_dict_field_nodes=dict_field_types,
    ...
    list_encoded_entities = build_node_list(ulog=ulog, sab=args.sab, yaml_dict_field_nodes=dict_field_entities,
    ...
    list_encoded_assays = build_node_list(ulog=ulog, sab=args.sab, yaml_dict_field_nodes=dict_field_assays,
    ...
    list_encoded_schemas = build_node_list(ulog=ulog, sab=args.sab, yaml_dict_field_nodes=dict_field_schemas,
    ...
    # field: type
    add_assertions(ulog=ulog, path=edgelist_path, dict_associations=dict_field_types,
                   list_fields=list_encoded_fields, list_objects=list_encoded_types,
                   predicate='has_datatype')
    # field:entity
    add_assertions(ulog=ulog, path=edgelist_path, dict_associations=dict_field_entities,
                   list_fields=list_encoded_fields, list_objects=list_encoded_entities,
                   predicate='used_in_entity')
    # field:assay
    add_assertions(ulog=ulog, path=edgelist_path, dict_associations=dict_field_assays,
                   list_fields=list_encoded_fields, list_objects=list_encoded_assays,
                   predicate='used_in_dataset')
    # field:schema
    add_assertions(ulog=ulog, path=edgelist_path, dict_associations=dict_field_schemas,
