...
This script is designed to align with the conversion logic executed in the sab2edgenode script--e.g., outputs to
owlnets_output, etc.

This means:
1. The UNIPROTKB CSV file will be extracted from GZ and downloaded to the OWL folder
   path, even though it is not an OWL file.
2. The OWLNETS output will be stored in the OWLNETS folder path.
...
def main():
    ...
    ulog = ubkgLogging(log_dir=log_dir, log_file='unprotkb2jkgen.log')

    args = getargs()

    # Get application configuration.
    cfgpath = os.path.join(os.path.dirname(os.getcwd()), 'generation_framework/translators/uniprotkb2jkgen/uniprotkb2jkgen.ini')
    ...
    if args.fetchnew:
        ...
        filepath = os.path.join(sab_jkg_dir, 'UNIPROTKB_all.tsv')
        ...
        hgncpath = os.path.join(sab_source_dir, 'HGNC.TSV')
