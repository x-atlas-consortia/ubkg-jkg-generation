...
def getvs(ulog: ubkgLogging, uext:ubkgExtract, path: str) -> pd.DataFrame:
    ...
    try:
        ...
        ulog.print_and_logger_info('REACTOME depends on the prior ingestion of information '
                                   'from the REACTOME_VS SAB. Run .sab2edgenode.sh for REACTOME_VS prior '
...
def main():
    ...
    ulog = ubkgLogging(log_dir=log_dir, log_file='reactome2jkgen.log')

    # Obtain runtime arguments.
    args = getargs()

    # Get application configuration.
    cfgpath = os.path.join(os.path.dirname(os.getcwd()), 'generation_framework/translators/reactome2jkgen/reactome2jkgen.ini')
