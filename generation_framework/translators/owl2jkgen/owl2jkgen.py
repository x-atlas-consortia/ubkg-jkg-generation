...
def get_owl_file(ulog: ubkgLogging, uextract: ubkgExtract, owl_dir: str, args: argparse.Namespace, usource: ubkgSources) -> str:
    ...
    if '.' not in working_file[len(working_file) - 5:len(working_file)]:
        ...
        working_file_new = args.owl_sab.lower() + '.owl'
...
def find_existing_owl_file(ulog: ubkgLogging, owl_dir: str, sab:str) -> str:
    ...
    if owl_files:
        ulog.print_and_logger_info(f'Using OWL file: {owl_files[0]}')
        return owl_files[0]

    if len(rdf_files) > 1:
        raise ValueError(f'Multiple RDF files found in {owl_dir}: {rdf_files}')
    if rdf_files:
        ulog.print_and_logger_info(f'Using RDF file: {rdf_files[0]}')
        return rdf_files[0]
...
def get_rdf_graph(ulog: ubkgLogging, usource: ubkgSources, owl_dir: str, owl_file: str, sab:str) -> Graph:
    ...
    try:
        ...
        convertedpath = os.path.join(owl_dir, f'{sab.lower()}.owl')
...
def do_file_housekeeping(ulog: ubkgLogging, args: argparse.Namespace):
    ...
    if not os.path.exists(owl_dir):
        ...
        if args.force_owl_download:
            ulog.print_and_logger_warning(f'Deleting prior input files in {owl_dir}')
            os.chdir(owl_dir)
...
def main():
    ...
    ulog = ubkgLogging(log_dir=log_dir, log_file='owl2jkgen.log')
